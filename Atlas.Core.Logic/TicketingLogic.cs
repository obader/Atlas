...
namespace Atlas.Core.Logic
{
    public class TicketingLogic
    {
                ...
                try
                {
                    var ticketsQuery =
                           (from ticket in ctx.Tickets
                            ...
                            && (
                                transactionId == 0
                                || (transactionId > 0 && ticket.TransactionId == transactionId.ToString())
                            )
                            && (
                                string.IsNullOrEmpty(mobileNumber)
                                || (!string.IsNullOrEmpty(mobileNumber) && ticket.MobileNumber == mobileNumber)
                            )
                            select ticket
                        );
            ...
            long ticketId, long transactionId, string mobileNumber, string bankTransactionId)
        {
            ...
            string reason = string.Empty;
            MasterTicket parentTicket = null;
            List<long> ticketIdsForTransaction = new List<long>();
            List<long> ticketIdsForMobile = new List<long>();
            ...
            using (var ctx = DM.TicketingEntities.ConnectToSqlServer(_connectionInfo))
            {
                try
                {
                    ...
                    foreach (var ticket in tickets)
                    {
                        ...
                        {
                            var dbParentTicket = ctx.Tickets.AsNoTracking().FirstOrDefault(p => p.TicketId == ticket.TicketParentId);
        ...
        public List<MasterTicket> GetTicketsByProfileId(string pUserId, int pCategoryId, int statusId, int profileId)
        {
            var lTickets = new List<MasterTicket>();
            string category = string.Empty;
            string statusCode = string.Empty;
        ...
        public MasterTicket GetTicketById(string pUserId, long pTicketId)
        {
            using (var ctx = DM.TicketingEntities.ConnectToSqlServer(_connectionInfo))
            {
                ...
                try
                {
                    ...
                    //get channel code
                    if (lTicket.ChannelId != null && lTicket.ChannelId != 0)
            ...
            int page, int itemsPerPage)
        {
            ...
            using (var ctx = DM.TicketingEntities.ConnectToSqlServer(_connectionInfo))
            {
                try
                {
                    ...
                    foreach (var ticket in tickets)
                    {
                        var transaction = ticket.TicketTransactions.First();
                        //if (transaction != null)
                        //{
                        if (ticket.CategoryId != null && ticket.CategoryId > 0)
                        {
